            ...
            for (int outer_ic_idx = 0; outer_ic_idx < {conv_layer_name}_outer_in_channel; outer_ic_idx++) {left_bracket}
                ...
                for (int global_weight_idx = 0; global_weight_idx < DIV_CEIL(global_weight_oc_num, OUT_CHANNEL_{global_weight_type}); global_weight_idx++) {left_bracket}
                    if (global_weight_idx < DIV_CEIL(global_weight_oc_num, OUT_CHANNEL_{global_weight_type}) - 1)
                        nnet::copy_weights_DDR2BRAM<{conv_layer_type}_DDR_weight_config, {global_weight_type}_config>({conv_layer_name}_w_0, {global_weight_name}[{conv_layer_name}_allocate_{global_weight_name}_start_idx+ global_weight_idx],
...
#pragma HLS unroll
                                    ...
                                    if (i_idx == 0) {left_bracket}
                                        if (outer_ic_idx == 0) {left_bracket}
                                            ...
                                            nnet::set_bias<{conv_layer_type}_set_bias_config>(local_feature_out_{conv_layer_type}[pe_idx], {conv_layer_name}_b_0+({conv_layer_name}_allocate_bias_start_idx+pe_idx + o_idx * {conv_layer_name}_inner_pe_parallel+ outer_oc_idx * {conv_layer_name}_block_out_channel));
                                            std::cout << "" ;
                                        {right_bracket}
                                        else {left_bracket}
                                            //restore partial sum
...
/////////////template_config /////////////
///{conv_layer_name}
struct DDR_feature_{conv_layer_input}_config : nnet::Feature_Memory {left_bracket}
	typedef FIX_INT20 feature_type;
	static const unsigned channel = {conv_layer_name}_in_channel;
	static const unsigned height = {conv_layer_name}_in_height;
	static const unsigned width = {conv_layer_name}_in_width;
{right_bracket};



struct {conv_layer_type}_set_bias_config : nnet::set_bias_config {left_bracket}
	typedef FIX_INT20 bias_type;
	typedef FIX_INT20 feature_type;

	static const unsigned channel = 1;
	static const unsigned height = OUT_HEIGHT_{conv_layer_type};
	static const unsigned width = OUT_WIDTH_{conv_layer_type};
{right_bracket};
