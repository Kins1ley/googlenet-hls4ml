    ...
    for (int outer_h_idx = 0; outer_h_idx < pool1_3x3_s2_outer_height; outer_h_idx++) {
        for (int outer_w_idx = 0; outer_w_idx < pool1_3x3_s2_outer_width; outer_w_idx++) {
            for (int outer_ic_idx = 0; outer_ic_idx < pool1_3x3_s2_outer_in_channel; outer_ic_idx++) {
                ...
                int inner_height = DIV_CEIL((DDR_block_in_feature_h_start_idx + pool1_3x3_s2_pad_top + global_block_in_feature_h_num + inner_pad_bottom - KERNEL_HEIGHT_AVGPOOL7x7_S1) / (STRIDE_AVGPOOL7x7_S1)+1
                    - DIV_CEIL(DDR_block_in_feature_h_start_idx + pool1_3x3_s2_pad_top, STRIDE_AVGPOOL7x7_S1),
                    OUT_HEIGHT_AVGPOOL7x7_S1);
                int inner_width = DIV_CEIL((DDR_block_in_feature_w_start_idx + pool1_3x3_s2_pad_left + global_block_in_feature_w_num + inner_pad_right - KERNEL_WIDTH_AVGPOOL7x7_S1) / (STRIDE_AVGPOOL7x7_S1)+1
                    - DIV_CEIL(DDR_block_in_feature_w_start_idx + pool1_3x3_s2_pad_left, STRIDE_AVGPOOL7x7_S1),
                    OUT_WIDTH_AVGPOOL7x7_S1);
                if (outer_h_idx == 0) {
                    inner_height = DIV_CEIL((DDR_block_in_feature_h_start_idx + global_block_in_feature_h_num + inner_pad_bottom + pool1_3x3_s2_pad_top - KERNEL_HEIGHT_AVGPOOL7x7_S1) / (STRIDE_AVGPOOL7x7_S1)+1
                        - DIV_CEIL(DDR_block_in_feature_h_start_idx, STRIDE_AVGPOOL7x7_S1),
                        OUT_HEIGHT_AVGPOOL7x7_S1);
                }
                if (outer_w_idx == 0) {
                    inner_width = DIV_CEIL((DDR_block_in_feature_w_start_idx + global_block_in_feature_w_num + inner_pad_right + pool1_3x3_s2_pad_left - KERNEL_WIDTH_AVGPOOL7x7_S1) / (STRIDE_AVGPOOL7x7_S1)+1
                        - DIV_CEIL(DDR_block_in_feature_w_start_idx, STRIDE_AVGPOOL7x7_S1),
                        OUT_WIDTH_AVGPOOL7x7_S1);
                }
                int inner_channel = DIV_CEIL(global_block_in_feature_c_num, pool1_3x3_s2_inner_pe_parallel * N_CHAN_AVGPOOL7x7_S1);
                //do inner loop
                for (int h_idx = 0; h_idx < inner_height; h_idx++) {
                    for (int w_idx = 0; w_idx < inner_width; w_idx++) {
                        for (int c_idx = 0; c_idx < inner_channel; c_idx++) {
                            ...
                            if (c_idx == inner_channel - 1) inner_pe_parallel = DIV_CEIL(global_block_in_feature_c_num - c_idx * pool1_3x3_s2_inner_pe_parallel * N_CHAN_AVGPOOL7x7_S1, N_CHAN_AVGPOOL7x7_S1);
#pragma HLS pipeline
                            for (int pe_idx = 0; pe_idx < inner_pe_parallel; pe_idx++) {
#pragma HLS unroll
                                //index of input feature in global BRAM
                                int global_in_feature_c_start_idx = (c_idx * pool1_3x3_s2_inner_pe_parallel + pe_idx) * N_CHAN_AVGPOOL7x7_S1;
                                ...
                                int global_out_feature_c_start_idx = (c_idx * pool1_3x3_s2_inner_pe_parallel + pe_idx) * N_CHAN_AVGPOOL7x7_S1;
                                ...
                                if ((c_idx == inner_channel - 1) && (pe_idx == inner_pe_parallel - 1)) {
                                    //handle the last iteration of the loop
                                    local_out_feature_c_num = global_block_in_feature_c_num - (c_idx * pool1_3x3_s2_inner_pe_parallel + pe_idx) * N_CHAN_AVGPOOL7x7_S1;
                                    local_in_feature_c_num = global_block_in_feature_c_num - (c_idx * pool1_3x3_s2_inner_pe_parallel + pe_idx) * N_CHAN_AVGPOOL7x7_S1;
                                }
                                // handle the situation that convolution does not start from the first element
                                if (outer_h_idx != 0) {
                                    global_in_feature_h_start_idx += (DDR_block_in_feature_h_start_idx + pool1_3x3_s2_pad_top) % STRIDE_AVGPOOL7x7_S1;
                                }
                                if (outer_w_idx != 0) {
                                    global_in_feature_w_start_idx += (DDR_block_in_feature_w_start_idx + pool1_3x3_s2_pad_left) % STRIDE_AVGPOOL7x7_S1;
                                }
                                ...
                                nnet::copy_features_g2l<global_feature_config, AVGPOOL7x7_S1_local_feature_in_config>(global_feature[pool1_3x3_s2_allocate_global_in_feature_start_idx + global_in_feature_c_start_idx / CHANNEL_FEATURE_GLOBAL], local_feature_in_AVGPOOL7x7_S1[pe_idx],
                                ...
                                nnet::copy_features_l2g<AVGPOOL7x7_S1_local_feature_out_config, global_feature_config>(local_feature_out_AVGPOOL7x7_S1[pe_idx], global_feature[pool1_3x3_s2_allocate_global_out_feature_start_idx + global_out_feature_c_start_idx / CHANNEL_FEATURE_GLOBAL],
                ...
                //copy out feature from BRAM to DRAM
                {
                    ...
                    int DDR_block_out_feature_h_start_idx = DIV_CEIL((DDR_block_in_feature_h_start_idx + pool1_3x3_s2_pad_top), STRIDE_AVGPOOL7x7_S1);
                    int DDR_block_out_feature_w_start_idx = DIV_CEIL((DDR_block_in_feature_w_start_idx + pool1_3x3_s2_pad_left), STRIDE_AVGPOOL7x7_S1);
                    ...
                    int DDR_block_out_feature_h_num = (DDR_block_in_feature_h_start_idx + pool1_3x3_s2_pad_top + global_block_in_feature_h_num - KERNEL_HEIGHT_AVGPOOL7x7_S1) / STRIDE_AVGPOOL7x7_S1 + 1 - DDR_block_out_feature_h_start_idx;
                    int DDR_block_out_feature_w_num = (DDR_block_in_feature_w_start_idx + pool1_3x3_s2_pad_left + global_block_in_feature_w_num - KERNEL_WIDTH_AVGPOOL7x7_S1) / STRIDE_AVGPOOL7x7_S1 + 1 - DDR_block_out_feature_w_start_idx;

                    if (outer_h_idx == pool1_3x3_s2_outer_height - 1) {
                        //handle the last iteration of the loop
                        DDR_block_out_feature_h_num = (DDR_block_in_feature_h_start_idx + pool1_3x3_s2_pad_top + pool1_3x3_s2_pad_bottom + global_block_in_feature_h_num - KERNEL_HEIGHT_AVGPOOL7x7_S1) / STRIDE_AVGPOOL7x7_S1 + 1 - DDR_block_out_feature_h_start_idx;
                    }
                    if (outer_w_idx == pool1_3x3_s2_outer_width - 1) {
                        //handle the last iteration of the loop
                        DDR_block_out_feature_w_num = (DDR_block_in_feature_w_start_idx + pool1_3x3_s2_pad_left + pool1_3x3_s2_pad_right + global_block_in_feature_w_num - KERNEL_WIDTH_AVGPOOL7x7_S1) / STRIDE_AVGPOOL7x7_S1 + 1 - DDR_block_out_feature_w_start_idx;
                    }
