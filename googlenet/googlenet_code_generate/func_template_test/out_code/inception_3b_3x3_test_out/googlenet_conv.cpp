				...
				for (int ic_idx = 0; ic_idx < conv1_7x7_s2_in_channel; ++ic_idx) {
					...
					for (int kh_idx = 0; kh_idx < conv1_7x7_s2_kernel_height; ++kh_idx) {
						for (int kw_idx = 0; kw_idx < conv1_7x7_s2_kernel_width; ++kw_idx) {
							if (oh_idx*STRIDE_CONV7x7_S2 + kh_idx - conv1_7x7_s2_pad_top < 0) continue;
							if (oh_idx*STRIDE_CONV7x7_S2 + kh_idx - conv1_7x7_s2_pad_top >= conv1_7x7_s2_in_height) continue;
							if (ow_idx*STRIDE_CONV7x7_S2 + kw_idx - conv1_7x7_s2_pad_left < 0) continue;
							if (ow_idx*STRIDE_CONV7x7_S2 + kw_idx - conv1_7x7_s2_pad_left >= conv1_7x7_s2_in_width) continue;
							outputs[oc_idx * conv1_7x7_s2_out_height * conv1_7x7_s2_out_width + oh_idx * conv1_7x7_s2_out_width + ow_idx] += (FIX_INT20)(inputs[ic_idx * conv1_7x7_s2_in_height * conv1_7x7_s2_in_width+(oh_idx*STRIDE_CONV7x7_S2 + kh_idx - conv1_7x7_s2_pad_top) * conv1_7x7_s2_in_width +ow_idx*STRIDE_CONV7x7_S2 + kw_idx - conv1_7x7_s2_pad_left]
...
int main(int argc, char const *argv[]) {
	...
	for (int ic = 0; ic < conv1_7x7_s2_in_channel; ++ic) {
		for (int ih = 0; ih < conv1_7x7_s2_in_height; ++ih) {
			for (int iw = 0; iw < conv1_7x7_s2_in_width; ++iw) {
				...
				std::cout << inputs[ic * conv1_7x7_s2_in_height * conv1_7x7_s2_in_width + ih * conv1_7x7_s2_in_width + iw] << ",";
			}std::cout << std::endl;
		}std::cout << std::endl;
	}
	//weights
	for (int o_idx = 0; o_idx < conv1_7x7_s2_out_channel; ++o_idx) {
		...
		std::cout << "bias " << bias[o_idx] << std::endl;
		for (int i_idx = 0; i_idx < conv1_7x7_s2_in_channel; ++i_idx) {
			for (int h_idx = 0; h_idx < conv1_7x7_s2_kernel_height; ++h_idx) {
				for (int w_idx = 0; w_idx < conv1_7x7_s2_kernel_width; ++w_idx) {
					...
					std::cout << weights[o_idx * conv1_7x7_s2_in_channel + i_idx][h_idx][w_idx] << ",";
				}std::cout << std::endl;
			}std::cout << std::endl;
		}std::cout << std::endl;
	}
	...
	for (int oc = 0; oc < conv1_7x7_s2_out_channel; ++oc) {
		for (int oh = 0; oh < conv1_7x7_s2_out_height; ++oh) {
			for (int ow = 0; ow < conv1_7x7_s2_out_width; ++ow) {
				std::cout.width(4);
				std::cout << (float)outputs_hardware[oc * conv1_7x7_s2_out_height * conv1_7x7_s2_out_width + oh * conv1_7x7_s2_out_width + ow];
			}
			std::cout << std::endl;
		}
		std::cout << std::endl;
	...
	std::cout << "software outputs: " << std::endl;
	for (int oc = 0; oc < conv1_7x7_s2_out_channel; ++oc) {
		for (int oh = 0; oh < conv1_7x7_s2_out_height; ++oh) {
			for (int ow = 0; ow < conv1_7x7_s2_out_width; ++ow) {
				std::cout.width(4);
				std::cout << (float)outputs_software[oc * conv1_7x7_s2_out_height * conv1_7x7_s2_out_width + oh * conv1_7x7_s2_out_width + ow];
			}
			std::cout << std::endl;
		}
		std::cout << std::endl;
	...
	if (diff < 0.1) {
		std::cout << "software outputs match hardware outputs! " << std::endl;
	}
	else {
		std::cout << "software outputs do not match hardware outputs! " << std::endl;
	}
