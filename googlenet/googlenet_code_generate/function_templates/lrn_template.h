...
for (int outer_h_idx = 0; outer_h_idx < {layer_name}_outer_height; outer_h_idx++) {{
    for (int outer_w_idx = 0; outer_w_idx < {layer_name}_outer_width; outer_w_idx++) {{
        for (int outer_ic_idx = 0; outer_ic_idx < {layer_name}_outer_in_channel; outer_ic_idx++) {{
            ...
            int DDR_block_in_feature_c_start_idx = outer_ic_idx * {layer_name}_block_interval_channel;
            ...
            {{
                ...
                if (outer_ic_idx == {layer_name}_outer_in_channel - 1) {{
                    global_block_in_feature_c_num = {layer_name}_in_channel - outer_ic_idx * {layer_name}_block_interval_channel;
                }}
            }}
            ...
            //dims of inner loop
            int inner_height = DIV_CEIL(global_block_in_feature_h_num, OUT_HEIGHT_{layer_type});
            int inner_width = DIV_CEIL(global_block_in_feature_w_num, OUT_WIDTH_{layer_type});
            int padding_channel_left = (outer_ic_idx == 0) ? {layer_name}_depth_radius : 0;
            int padding_channel_right = (outer_ic_idx == ({layer_name}_outer_in_channel - 1)) ? {layer_name}_depth_radius : 0;
            int inner_channel = DIV_CEIL(global_block_in_feature_c_num - 2 * {layer_name}_depth_radius + padding_channel_left + padding_channel_right, {layer_name}_inner_pe_parallel * (N_CHAN_{layer_type} - 2 * {layer_name}_depth_radius));
            //do inner loop
            for (int h_idx = 0; h_idx < inner_height; h_idx++) {{
                for (int w_idx = 0; w_idx < inner_width; w_idx++) {{
                    for (int c_idx = 0; c_idx < inner_channel; c_idx++) {{
                        ...
                        //if (c_idx == inner_channel - 1) inner_pe_parallel = DIV_CEIL(global_block_in_feature_c_num - c_idx * {layer_name}_inner_pe_parallel * (N_CHAN_{layer_type} - 2 * {layer_name}_depth_radius), (N_CHAN_{layer_type} - 2 * {layer_name}_depth_radius));
#pragma HLS pipeline
                        for (int pe_idx = 0; pe_idx < inner_pe_parallel; pe_idx++) {{
#pragma HLS unroll
                            //index of input feature in global BRAM
                            int global_in_feature_c_start_idx = (c_idx * {layer_name}_inner_pe_parallel + pe_idx) * (N_CHAN_{layer_type} - 2 * {layer_name}_depth_radius) - padding_channel_left;
                            int global_in_feature_h_start_idx = h_idx * OUT_HEIGHT_{layer_type}; //
                            int global_in_feature_w_start_idx = w_idx * OUT_WIDTH_{layer_type};//
                            ...
                            int global_out_feature_c_start_idx = (c_idx * {layer_name}_inner_pe_parallel + pe_idx) * (N_CHAN_{layer_type} - 2 * {layer_name}_depth_radius);
                            ...
                            int local_out_feature_c_num = N_CHAN_{layer_type} - 2 * {layer_name}_depth_radius;
                            ...
                            if (h_idx == inner_height - 1) {{
                                ...
                                local_in_feature_h_num = global_block_in_feature_h_num - h_idx * OUT_HEIGHT_{layer_type};
                                local_out_feature_h_num = local_in_feature_h_num;
                            }}

                            if (w_idx == inner_width - 1) {{
                                //handle the last iteration of the loop and padding
                                local_in_feature_w_num = global_block_in_feature_w_num - w_idx * OUT_WIDTH_{layer_type};
                                local_out_feature_w_num = local_in_feature_w_num;
                            }}

                            if ((c_idx == inner_channel - 1) && (pe_idx == (inner_pe_parallel - 1))) {{
                                //handle the last iteration of the loop
                                local_in_feature_c_num = global_block_in_feature_c_num + padding_channel_right + padding_channel_left - (c_idx * {layer_name}_inner_pe_parallel + pe_idx) * (N_CHAN_{layer_type} - 2 * {layer_name}_depth_radius);
                                local_out_feature_c_num = local_in_feature_c_num + padding_channel_right - 2 * {layer_name}_depth_radius;
                            }}
                            if ((c_idx * {layer_name}_inner_pe_parallel + pe_idx) == 0) {{
                            ...
                            nnet::{layer_type}<{layer_type}_config>(local_feature_in_{layer_type}[pe_idx], local_feature_out_{layer_type}[pe_idx], bias, alpha, beta, {layer_name}_depth_radius);
            ...
            {{
                ...
                int DDR_block_out_feature_c_start_idx = DDR_block_in_feature_c_start_idx + {layer_name}_depth_radius - padding_channel_left;
                ...
                int DDR_block_out_feature_c_num = global_block_in_feature_c_num - 2 * {layer_name}_depth_radius + padding_channel_left + padding_channel_right;
                ...
                if (outer_ic_idx == {layer_name}_outer_in_channel - 1) {{
                    //handle the last iteration of the loop
                    DDR_block_out_feature_c_num = {layer_name}_in_channel - DDR_block_out_feature_c_start_idx;
                }}
...
const int {layer_name}_allocate_global_in_feature_num = 1;//multi global BRAM is not supportted yet
const int {layer_name}_allocate_global_out_feature_start_idx = 1;
const int {layer_name}_allocate_global_out_feature_num = 1;
///overlapped features between blocks
const int {layer_name}_block_overlap_channel = 2 * {layer_name}_depth_radius;
///number of blocks(the dims of the outer loop)
const int {layer_name}_outer_in_channel = DIV_CEIL({layer_name}_in_channel, {layer_name}_allocate_global_in_feature_num * (CHANNEL_FEATURE_GLOBAL - 2 * {layer_name}_depth_radius));
const int {layer_name}_outer_height = DIV_CEIL({layer_name}_in_height, HEIGHT_FEATURE_GLOBAL);
const int {layer_name}_outer_width = DIV_CEIL({layer_name}_in_width, WIDTH_FEATURE_GLOBAL);
///interval between blocks
const int {layer_name}_block_interval_channel = DIV_CEIL({layer_name}_in_channel, {layer_name}_outer_in_channel);
//const int {layer_name}_block_interval_channel = {layer_name}_allocate_global_in_feature_num*( CHANNEL_FEATURE_GLOBAL- 2 * {layer_name}_depth_radius);//the spacing between blocks
const int {layer_name}_block_interval_height = HEIGHT_FEATURE_GLOBAL;
const int {layer_name}_block_interval_width = WIDTH_FEATURE_GLOBAL;
///dim of blocks
const int {layer_name}_block_in_height = {layer_name}_block_interval_height;
const int {layer_name}_block_in_width = {layer_name}_block_interval_width;
const int {layer_name}_block_in_channel = {layer_name}_block_interval_channel + {layer_name}_block_overlap_channel;
///set parallism
const int {layer_name}_inner_pe_parallel = NUM_PE_{layer_type};//prallelism not supported
/////////////allocate_config/////////////