...
const int {lrn_layer_name}_block_overlap_channel = 2 * {lrn_layer_name}_deepth_radius;
///number of blocks(the dims of the outer loop)
const int {lrn_layer_name}_outer_in_channel = DIV_CEIL({lrn_layer_name}_in_channel, {lrn_layer_name}_allocate_global_in_feature_num * (CHANNEL_FEATURE_GLOBAL - 2 * {lrn_layer_name}_deepth_radius));
...
const int {lrn_layer_name}_block_interval_channel = {lrn_layer_name}_allocate_global_in_feature_num * (CHANNEL_FEATURE_GLOBAL - 2 * {lrn_layer_name}_deepth_radius);//the spacing between blocks
