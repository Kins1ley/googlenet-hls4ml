        ...
        for (int outer_ic_idx = 0; outer_ic_idx < {lrn_layer_name}_outer_in_channel; outer_ic_idx++) {left_bracket}
            ...
            for (int global_in_feature_idx = 0; global_in_feature_idx < DIV_CEIL(global_block_in_feature_c_num, CHANNEL_FEATURE_GLOBAL); global_in_feature_idx++) {left_bracket}
                if (global_in_feature_idx < DIV_CEIL(global_block_in_feature_c_num, CHANNEL_FEATURE_GLOBAL) - 1) {left_bracket}
                    ...
                    nnet::copy_features_DDR2BRAM<DDR_feature_{lrn_layer_input}_config, global_feature_config>({lrn_layer_input}, global_feature[{lrn_layer_name}_allocate_global_in_feature_start_idx + global_in_feature_idx],
            ...
            int padding_channel_left = (outer_ic_idx == 0) ? {lrn_layer_name}_depth_radius : 0;
            int padding_channel_right = (outer_ic_idx == ({lrn_layer_name}_outer_in_channel - 1)) ? {lrn_layer_name}_depth_radius : 0;
            int inner_channel = DIV_CEIL(global_block_in_feature_c_num - 2 * {lrn_layer_name}_depth_radius + padding_channel_left + padding_channel_right, {lrn_layer_name}_inner_pe_parallel * (N_CHAN_{lrn_layer_type} - 2 * {lrn_layer_name}_depth_radius));
            //do inner loop
            for (int h_idx = 0; h_idx < inner_height; h_idx++) {left_bracket}
                for (int w_idx = 0; w_idx < inner_width; w_idx++) {left_bracket}
                    for (int c_idx = 0; c_idx < inner_channel; c_idx++) {left_bracket}
                        ...
                        if (c_idx == inner_channel - 1) inner_pe_parallel = DIV_CEIL(global_block_in_feature_c_num - c_idx * {lrn_layer_name}_inner_pe_parallel * (N_CHAN_{lrn_layer_type} - 2 * {lrn_layer_name}_depth_radius), (N_CHAN_{lrn_layer_type} - 2 * {lrn_layer_name}_depth_radius));
#pragma HLS pipeline
                        for (int pe_idx = 0; pe_idx < inner_pe_parallel; pe_idx++) {left_bracket}
#pragma HLS unroll
                            //index of input feature in global BRAM
                            int global_in_feature_c_start_idx = (c_idx * {lrn_layer_name}_inner_pe_parallel + pe_idx) * (N_CHAN_{lrn_layer_type} - 2 * {lrn_layer_name}_depth_radius) - padding_channel_left;
                            ...
                            int global_out_feature_c_start_idx = (c_idx * {lrn_layer_name}_inner_pe_parallel + pe_idx) * (N_CHAN_{lrn_layer_type} - 2 * {lrn_layer_name}_depth_radius);
                            ...
                            int local_out_feature_c_num = N_CHAN_{lrn_layer_type} - 2 * {lrn_layer_name}_depth_radius;
                            ...
                            if ((c_idx == inner_channel - 1) && (pe_idx == inner_pe_parallel - 1)) {left_bracket}
                                //handle the last iteration of the loop
                                local_in_feature_c_num = global_block_in_feature_c_num + padding_channel_left - (c_idx * {lrn_layer_name}_inner_pe_parallel + pe_idx) * (N_CHAN_{lrn_layer_type} - 2 * {lrn_layer_name}_depth_radius);
                                local_out_feature_c_num = local_in_feature_c_num + padding_channel_right - 2 * {lrn_layer_name}_depth_radius;
                            {right_bracket}
                            ...
                            nnet::{lrn_layer_type}<{lrn_layer_type}_config>(local_feature_in_{lrn_layer_type}[pe_idx], local_feature_out_{lrn_layer_type}[pe_idx], bias, alpha, beta, {lrn_layer_name}_depth_radius);
            ...
            {left_bracket}
                ...
                int DDR_block_out_feature_c_start_idx = DDR_block_in_feature_c_start_idx + {lrn_layer_name}_depth_radius - padding_channel_left;
                ...
                int DDR_block_out_feature_c_num = global_block_in_feature_c_num - 2 * {lrn_layer_name}_depth_radius + padding_channel_left + padding_channel_right;
                ...
                for (int global_out_feature_idx = 0; global_out_feature_idx < DIV_CEIL(global_block_in_feature_c_num, CHANNEL_FEATURE_GLOBAL); global_out_feature_idx++) {left_bracket}
                    if (global_out_feature_idx < DIV_CEIL(DDR_block_out_feature_c_num, CHANNEL_FEATURE_GLOBAL) - 1)
                        nnet::copy_features_BRAM2DDR<global_feature_config, DDR_feature_{lrn_layer_output}_config>(global_feature[{lrn_layer_name}_allocate_global_out_feature_start_idx + global_out_feature_idx], {lrn_layer_output},
...
/////////////template_config/////////////
///{lrn_layer_name}
struct DDR_feature_{lrn_layer_output}_config : nnet::Feature_Memory {left_bracket}
