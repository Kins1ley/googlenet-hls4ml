...
for (int outer_h_idx = 0; outer_h_idx < {layer_name}_outer_height; outer_h_idx++) {{
	for (int outer_w_idx = 0; outer_w_idx < {layer_name}_outer_width; outer_w_idx++) {{
		for (int outer_oc_idx = 0; outer_oc_idx < {layer_name}_outer_out_channel; outer_oc_idx++) {{
			for (int outer_ic_idx = 0; outer_ic_idx < {layer_name}_outer_in_channel; outer_ic_idx++) {{
				...
				int DDR_weight_ic_start_idx = outer_ic_idx * {layer_name}_block_in_channel;
				int DDR_weight_oc_start_idx = outer_oc_idx * {layer_name}_block_out_channel;
				int global_weight_ic_num = {layer_name}_block_in_channel;
				int global_weight_oc_num = {layer_name}_block_out_channel;
				...
				{{
					...
					if (outer_oc_idx == {layer_name}_outer_out_channel - 1) {{
						global_weight_oc_num = {layer_name}_out_channel - outer_oc_idx * {layer_name}_block_out_channel;
					}}
					if (outer_ic_idx == {layer_name}_outer_in_channel - 1) {{
						...
						global_weight_ic_num = {layer_name}_in_channel - outer_ic_idx * {layer_name}_block_in_channel;
					}}
				}}
				...
				for (int global_weight_idx = 0; global_weight_idx < DIV_CEIL(global_weight_oc_num, OUT_CHANNEL_{global_weight_type}); global_weight_idx++) {{
					if (global_weight_idx < DIV_CEIL(global_weight_oc_num, OUT_CHANNEL_{global_weight_type}) - 1)
						nnet::copy_weights_DDR2BRAM<DDR_weight_{layer_name}_config, {global_weight_type}_config>({DDR_weight_type}, {global_weight_name}[{layer_name}_allocate_{global_weight_name}_start_idx + global_weight_idx],
							DDR_weight_oc_start_idx + global_weight_idx * OUT_CHANNEL_{global_weight_type}, OUT_CHANNEL_{global_weight_type},
							{layer_name}_kernel_channel_DDR_offset + DDR_weight_ic_start_idx, global_weight_ic_num);
					else
						nnet::copy_weights_DDR2BRAM<DDR_weight_{layer_name}_config, {global_weight_type}_config>({DDR_weight_type}, {global_weight_name}[{layer_name}_allocate_{global_weight_name}_start_idx + global_weight_idx],
				...
				int inner_out_channel = DIV_CEIL(global_weight_oc_num, {layer_name}_inner_pe_parallel * OUT_CHAN_{layer_type});
				int inner_in_channel = DIV_CEIL(global_weight_ic_num, IN_CHAN_{layer_type});
				//do inner loop
				for (int h_idx = 0; h_idx < inner_height; h_idx++) {{
					for (int w_idx = 0; w_idx < inner_width; w_idx++) {{
						for (int o_idx = 0; o_idx < inner_out_channel; o_idx++) {{
							...
							if (o_idx == inner_out_channel - 1) inner_pe_parallel = global_weight_oc_num - o_idx * {layer_name}_inner_pe_parallel;
...
#pragma HLS unroll
									...
									int global_weight_ic_start_idx = i_idx * IN_CHAN_{layer_type};
									int global_weight_oc_start_idx = (o_idx * {layer_name}_inner_pe_parallel + pe_idx) * OUT_CHAN_{layer_type};
									int local_weight_ic_num = IN_CHAN_{layer_type};
									int local_weight_oc_num = OUT_CHAN_{layer_type};

									//index of input feature in global BRAM
									int global_in_feature_c_start_idx = i_idx * IN_CHAN_{layer_type};
									...
									int local_in_feature_c_num = IN_CHAN_{layer_type};
									...
									if (o_idx == inner_out_channel - 1) {{
										//handle the last iteration of the loop
										local_weight_oc_num = global_weight_oc_num - o_idx * OUT_CHAN_{layer_type} * {layer_name}_inner_pe_parallel - OUT_CHAN_{layer_type} * pe_idx;
									}}
									if (i_idx == inner_in_channel - 1) {{
										//handle the last iteration of the loop
										local_in_feature_c_num = global_block_in_feature_c_num - i_idx * IN_CHAN_{layer_type};
										local_weight_ic_num = global_block_in_feature_c_num - i_idx * IN_CHAN_{layer_type};
									}}
									...
									if (i_idx == 0) {{
										if (outer_ic_idx == 0) {{
											...
											nnet::clear_buffer<{layer_type}_local_feature_out_config>(local_feature_out_{layer_type}[pe_idx]);
											nnet::set_bias<{layer_type}_set_bias_config>(local_feature_out_{layer_type}[pe_idx], DDR_bias + {layer_name}_bias_DDR_offset + ({layer_name}_allocate_bias_start_idx + pe_idx + o_idx * {layer_name}_inner_pe_parallel + outer_oc_idx * {layer_name}_block_out_channel));
											//std::cout << "";
										}}
										else {{
											//restore partial sum
											...
											nnet::clear_buffer<{layer_type}_local_feature_out_config>(local_feature_out_{layer_type}[pe_idx]);
											nnet::copy_features_g2l<global_feature_config, {layer_type}_local_feature_out_config>(global_feature[{layer_name}_allocate_global_out_feature_start_idx + global_out_feature_c_start_idx / CHANNEL_FEATURE_GLOBAL], local_feature_out_{layer_type}[pe_idx],
									...
									nnet::copy_weights_g2l<{global_weight_type}_config, {layer_type}_local_weight_config>({global_weight_name}[{layer_name}_allocate_{global_weight_name}_start_idx + global_weight_oc_start_idx / OUT_CHANNEL_{global_weight_type}], local_weight_{layer_type}[pe_idx],
									...
									nnet::{pe_name}<conv2d_config_{layer_type}>(local_feature_in_{layer_type}[pe_idx], local_weight_{layer_type}[pe_idx][0], local_feature_out_{layer_type}[pe_idx][0]);

									if (i_idx == inner_in_channel - 1) {{
										//copy output feature from local BRAM to global BRAM
										if (outer_ic_idx == {layer_name}_outer_in_channel - 1) {{
											nnet::relu_inplace<relu_conv2d_config_{layer_type}>(local_feature_out_{layer_type}[pe_idx]);
										}}
				...
				{{
					...
					for (int global_out_feature_idx = 0; global_out_feature_idx < DIV_CEIL(global_block_in_feature_c_num, CHANNEL_FEATURE_GLOBAL); global_out_feature_idx++) {{
						if (global_out_feature_idx < DIV_CEIL(DDR_block_out_feature_c_num, CHANNEL_FEATURE_GLOBAL) - 1)
							nnet::copy_features_BRAM2DDR<global_feature_config, DDR_feature_{layer_output_name}_config>(global_feature[{layer_name}_allocate_global_out_feature_start_idx], {DDR_out_feature},
...
const int {layer_name}_allocate_global_in_feature_num = 2;
const int {layer_name}_allocate_{global_weight_name}_start_idx = 0;
const int {layer_name}_allocate_{global_weight_name}_num = 2;
const int {layer_name}_allocate_global_out_feature_start_idx = 2;
const int {layer_name}_allocate_global_out_feature_num = 2;
const int {layer_name}_allocate_bias_start_idx = 0;
...
const int {layer_name}_outer_in_channel = DIV_CEIL({layer_name}_in_channel, {layer_name}_allocate_global_in_feature_num*CHANNEL_FEATURE_GLOBAL < IN_CHANNEL_{global_weight_type} ? {layer_name}_allocate_global_in_feature_num * CHANNEL_FEATURE_GLOBAL : IN_CHANNEL_{global_weight_type});
...
const int {layer_name}_block_in_channel = MIN({layer_name}_allocate_global_in_feature_num*CHANNEL_FEATURE_GLOBAL , IN_CHANNEL_{global_weight_type});
///set parallism
const int {layer_name}_inner_pe_parallel = NUM_PE_{layer_type};
///dim of kernels
const int {layer_name}_block_out_channel = MIN({layer_name}_allocate_global_out_feature_num*CHANNEL_FEATURE_GLOBAL , {layer_name}_allocate_{global_weight_name}_num*OUT_CHANNEL_{global_weight_type});
const int {layer_name}_outer_out_channel = DIV_CEIL({layer_name}_kernel_num, {layer_name}_block_out_channel);//outer loop
/////////////allocate_config/////////////